...
function update_words(base, words, brushScale, cell_width) {
    var word = base.selectAll(".word").data(words);
    word.exit().remove();
    ...
    word.select('text')
        .attr({
            "transform": function (d) {
                ...
                // if (tl > (cell_width - 3)) {
                //     return "translate(2,0)scale(" + (cell_width - 3) / tl + ",1)";
                // } else {
    ...
    base.selectAll(".separator_line").data([0]).enter().append("line").attr({
        "class": "separator_line",
        x1: function (d) {return brushScale(d) - 1},
        x2: function (d) {return brushScale(d) - 1},
        y1: -5,
        y2: 10 + 5
    }).style({
        'pointer-events': 'none',
